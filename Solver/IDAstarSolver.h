...
#ifndef RUBIKSCUBE3BY3_IDASTARSOLVER_H
#define RUBIKSCUBE3BY3_IDASTARSOLVER_H
...
private:
    ...
    bool isInvmove(int curMove,int movePerform) {
        return (RubiksCube::MOVE(movePerform) ==RubiksCube::getInvMove(curMove));
    }
...
//        priority_queue contains pair(Node, move done to reach that)
        ...
        pq.push(make_pair(start, -1));//check
        int next_bound = 100;
        while (!pq.empty()) {
            auto p = pq.top();
            Node node = p.first;
            int lastMove=p.second;
            pq.pop();

            if (visited[node.cube]) continue;

            visited[node.cube] = true;
            if (lastMove!=-1) move_done[node.cube] = RubiksCube::MOVE(lastMove);

            if (node.cube.isSolved()) return make_pair(node.cube, bound);
            node.depth++;
            for (int i = 0; i < 18; i++) {
                if (lastMove!=-1 && isInvmove(lastMove,i)) continue;
...
#endif //RUBIKSCUBE3BY3_IDASTARSOLVER_H
