...
int RubiksCube::getColorInt(const char ch) {
    switch (ch) {
        case 'W': return 0;
        case 'G': return 1;
        case 'R': return 2;
...
RubiksCube::MOVE RubiksCube::getInvMove(const int idx) {
    int invMove;
    if (idx&1) {
        //odd
        invMove=(idx-1)%3;
    ...
    return RubiksCube::MOVE(invMove);
}

string RubiksCube::getSide(const int idx) {
    switch (idx) {
        case 0: return "UP";
        case 1: return "Left";
        case 2: return "Front";
