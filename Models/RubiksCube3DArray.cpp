...
public:
    ...
    bool isValid() const override {
        //color counting
        int cntColor[125]={};
        string colors="WGRBOY";
        for (int i=0;i<6;i++) {
            for (int j=0;j<3;j++) {
                for (int k=0;k<3;k++) {
                    cntColor[cube[i][j][k]-'A']++;
                }
            }
        }

        for (auto c:colors) {
            if (cntColor[c-'A']<9 || cntColor[c-'A']>9) return false;
        }

        //edges[edge number]->{face1,r1,c1,face2,r2,c2}
        for (int i=0;i<12;i++) {
            int color1=cube[edges[i][0]][edges[i][1]][edges[i][2]];
            int color2=cube[edges[i][3]][edges[i][4]][edges[i][5]];
            if (color1==color2) return false;
            if (color1=='W' && color2=='Y' || color1=='Y' && color2=='W') return false;
            if (color1=='R' && color2=='O' || color1=='O' && color2=='R') return false;
            if (color1=='B' && color2=='G' || color1=='G' && color2=='B') return false;
        }
        return true;
    }
...
struct Hash3d {
    size_t operator()(const RubiksCube3dArray &r1) const {
        ...
        for (int i = 0; i < 6; i++) {
            for (int j = 0; j < 3; j++) {
                for (int k = 0; k < 3; k++) {
                    hash1=(hash1*base1 + (r1.cube[i][j][k]-'A'+1))%mod1;
                    hash2=(hash2*base2 + (r1.cube[i][j][k]-'A'+1))%mod2;
                }
            }
        }
        return (static_cast<size_t>(hash1) << 32) |  static_cast<size_t>(hash2);
    }
};